...
from llmtuner.model.peft.tuners.tuners_utils import BaseTunerLayer, check_adapters_to_merge
from llmtuner.model.peft.utils.integrations import gather_params_ctx
from llmtuner.model.peft.utils.other import transpose
...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_moe_A", "lora_moe_B", "lora_embedding_A", "lora_embedding_B")
    ...
    def update_layer(
        self, adapter_name, r, lora_alpha, lora_dropout, lora_nums, init_lora_weights, use_rslora, use_dora: bool = False
    ):
        ...
        self.lora_nums[adapter_name] = lora_nums 
        ...
        self.lora_route = nn.Linear(self.in_features, lora_nums - 1, bias=False) 
        lora_A = nn.Parameter(torch.zeros(lora_nums, self.in_features, r))  
        lora_B = nn.Parameter(torch.zeros(lora_nums, r, self.out_features))  
        self.lora_moe_A.update(nn.ParameterDict({adapter_name: lora_A})) 
        self.lora_moe_B.update(nn.ParameterDict({adapter_name: lora_B}))
        
        self.w_noise = nn.Parameter(torch.zeros(self.in_features, lora_nums - 1 ), requires_grad=True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        nn.init.kaiming_uniform_(self.w_noise, a=math.sqrt(5))
        ...
        if adapter_name in self.lora_moe_A.keys():
            if init_lora_weights is True:
                ...
                nn.init.normal_(self.lora_moe_A[adapter_name], std=1 / self.r[adapter_name])
            else:
    ...
    def loftq_init(self, adapter_name):
        ...
        if adapter_name in self.lora_embedding_A.keys():
            # initialize a the same way as the default for nn.linear and b to zero
            self.lora_embedding_A[adapter_name].weight.data = lora_moe_A
            self.lora_embedding_B[adapter_name].weight.data = lora_moe_B
    ...
    def dora_init(self, adapter_name: str) -> None:
        lora_A = self.lora_moe_A[adapter_name]
        lora_B = self.lora_moe_B[adapter_name]
    ...
    #+++
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters: 
                ...
                raw_noise_stddev = x @ self.w_noise #torch.Size([8, 168, 4096])*torch.Size([4096, 2])
                noise_stddev = ((self.softplus(raw_noise_stddev))) #torch.Size([8, 168, 2])
                
                raw_logits = self.lora_route(x) + (torch.randn_like(noise_stddev) * noise_stddev)  
                #x:b*len*d   路由：d*lora_nums-1   得到权重:b*len*lora_nums-1
                route_weight = nn.functional.softmax(raw_logits, dim=-1, dtype=torch.float16).to(result.dtype)
                x = x.to(self.lora_moe_A[active_adapter].dtype)
                out = self.lora_dropout[active_adapter](x)
                
                scaling = self.scaling[active_adapter]
                out = torch.einsum('bnd,edh->bneh', out, self.lora_moe_A[active_adapter])
                out = torch.einsum('bneh,ehd->bned', out, self.lora_moe_B[active_adapter]) * scaling
                hide_state = out.clone()
                out = torch.cat([hide_state[:,:,0,:].unsqueeze(2), hide_state[:,:,1:,:] * route_weight.unsqueeze(-1)], dim=2) 
        ...
        # if self.active_adapter not in self.lora_moe_A.keys():
        ...
        #     out = torch.einsum('bned,be->bned', out, route_weight)
...
class Conv2d(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        """
        device = self.lora_moe_B[adapter].weight.device
        dtype = self.lora_moe_A[adapter].weight.dtype
