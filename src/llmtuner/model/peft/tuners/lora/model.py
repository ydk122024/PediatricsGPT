...
from llmtuner.model.peft.import_utils import is_bnb_4bit_available, is_bnb_available
from llmtuner.model.peft.tuners.tuners_utils import BaseTuner, BaseTunerLayer, check_target_module_exists, onload_layer
from llmtuner.model.peft.utils import (
...
from llmtuner.model.peft.utils.merge_utils import dare_linear, dare_ties, magnitude_prune, task_arithmetic, ties
...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        from llmtuner.model.peft.tuners.adalora import AdaLoraLayer
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                if adapter_name in target.lora_moe_A:
                    target_lora_A = target.lora_moe_A[adapter_name].weight
                    target_lora_B = target.lora_moe_B[adapter_name].weight
                ...
                if combination_type == "cat":
                    ...
                    for adapter, weight in zip(adapters, weights):
                        if adapter in target.lora_moe_A:
                            current_adapter_lora_A = target.lora_moe_A[adapter].weight
                            current_adapter_lora_B = target.lora_moe_B[adapter].weight
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        for adapter, weight in zip(adapters, weights):
            if adapter in target.lora_moe_A or adapter in target.lora_embedding_A:
